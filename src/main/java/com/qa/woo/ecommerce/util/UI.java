...
//This class was copied from the Git for the purposes of preserving formatting
...
public class UI {
	...
	public Product productInput() {
		...
		try  {
			...
			product = Product.builder().id(id).name(name).price(price).rating(rating).category(category).inStock(inStock).discountPercentage(discountPercentage).deliveryMode(deliveryMode).isReturnAccepted(isReturnAccepted).sellerName(sellerName).build();
